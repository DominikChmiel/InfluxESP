...
using sensor_data = struct sensor_data {
	...
	String toString() {
		char full_str[128];
		snprintf(full_str, sizeof(full_str), "temperature=%d.%03d,pressure=%d.%02d,humidity=%d.%02d",
			getTemp() / 1000, getTemp() % 1000,
			getPress() / 100, getPress() % 100,
			getHum() / 10000, getHum() % 10000);
